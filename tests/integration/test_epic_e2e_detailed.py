...
class TestEpicDetailedValidation:
    ...
    def test_patient_demographics(self, epic_bundle):
        ...
        # Find Patient
        patient = next(
            (e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Patient"),
            None,
        )
        ...
        assert first_identifier.value == "900646017", "First identifier must have value '900646017'"
    ...
    def test_problem_pneumonia(self, epic_bundle):
        ...
        # Find all Conditions
        conditions = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Condition"
        ]
        ...
        assert pneumonia is not None, (
            "Must have Condition with SNOMED code 385093006 (community acquired pneumonia)"
        )

        # EXACT check: Code text
        assert "pneumonia" in pneumonia.code.text.lower(), "Condition must mention 'pneumonia'"
        ...
        for allergy in allergies:
            if allergy.code and allergy.code.coding:
                for coding in allergy.code.coding:
                    if (
                        coding.code == "000476" or "penicillin" in coding.display.lower()
                        if coding.display
                        else False
                    ):
                        penicillins = allergy
    ...
    def test_has_observations(self, epic_bundle):
        ...
        observations = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Observation"
        ]
    ...
    def test_encounter_ambulatory(self, epic_bundle):
        ...
        # Find all Encounters
        encounters = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Encounter"
        ]
            ...
            if prac.identifier:
                for identifier in prac.identifier:
                    if (
                        ...
                        and identifier.value == "7603710774"
                    ):
                        dr_test = prac
                        break
    ...
    def test_diagnostic_report_leukocytes(self, epic_bundle):
        ...
        assert leukocytes is not None, (
            "Must have DiagnosticReport with LOINC code 6690-2 (Leukocytes count)"
        )
        ...
        loinc_coding = next(
            (c for c in leukocytes.code.coding if c.system == "http://loinc.org"), None
        )
    ...
    def test_medication_statement_albuterol(self, epic_bundle):
        ...
        assert albuterol is not None, (
            "Must have MedicationStatement with RxNorm code 1360201 (Albuterol)"
        )
        ...
        rxnorm_coding = next(
            (
                c
                for c in albuterol.medicationCodeableConcept.coding
    ...
    def test_all_medication_requests_have_identifiers(self, epic_bundle):
        ...
        for med_statement in med_statements:
            assert med_statement.identifier is not None, "MedicationStatement must have identifier"
            ...
            assert identifier.system is not None, "MedicationStatement identifier must have system"
            assert identifier.value is not None, "MedicationStatement identifier must have value"
    ...
    def test_medication_statements_reference_patient(self, epic_bundle):
        ...
        for ms in med_statements:
            ...
            assert ms.subject.reference is not None, (
    ...
    def test_all_procedures_have_status(self, epic_bundle):
        ...
        procedures = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Procedure"
        ]
    ...
    def test_observations_have_complete_value_quantities(self, epic_bundle):
        ...
        observations_with_quantity = [
            obs
            for obs in observations
            if hasattr(obs, "valueQuantity")
            and obs.valueQuantity is not None
            and hasattr(obs.valueQuantity, "unit")
            and obs.valueQuantity.unit is not None
        ]
    ...
    def test_composition_instant_timezone_exact(self, epic_bundle):
        ...
        compositions = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Composition"
        ]
