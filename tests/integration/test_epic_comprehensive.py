...
class TestEpicComprehensive:
    ...
    def test_condition_pneumonia_exact_values(self, epic_bundle):
        ...
        for cond in conditions:
            ...
            if (
                ...
                and any(coding.get("code") == "385093006" for coding in c["code"]["coding"])
            ):
                pneumonia = c
                break
        ...
        assert any(cat["coding"][0]["code"] == "problem-list-item" for cat in pneumonia["category"])
    ...
    def test_allergy_penicillins_exact_values(self, epic_bundle):
        ...
        for allergy in allergies:
            ...
            if (
                ...
                and any(coding.get("code") == "000476" for coding in a["code"]["coding"])
            ):
                penicillins = a
                break
        ...
        fdb_coding = next(
            (c for c in penicillins["code"]["coding"] if c.get("code") == "000476"), None
        )
    ...
    def test_observation_wbc_exact_values(self, epic_bundle):
        ...
        loinc_coding = next(
            (c for c in wbc["code"]["coding"] if c["system"] == "http://loinc.org"), None
        )
        ...
        # Exact category
        assert any(cat["coding"][0]["code"] == "laboratory" for cat in wbc["category"])
    ...
    def test_diagnostic_report_exact_values(self, epic_bundle):
        ...
        for report in reports:
            ...
            if (
                "code" in r
                and r["code"]
                and "coding" in r["code"]
                and any(coding.get("code") == "6690-2" for coding in r["code"]["coding"])
            ):
                wbc_report = r
                break
        ...
        if wbc_report:
            ...
            assert (
                wbc_report["category"][0]["coding"][0]["system"]
                == "http://terminology.hl7.org/CodeSystem/v2-0074"
            )

            # Exact code
            loinc_coding = next(
                (c for c in wbc_report["code"]["coding"] if c["system"] == "http://loinc.org"), None
            )
            assert loinc_coding["code"] == "6690-2"
            assert "Leukocytes" in loinc_coding["display"]
    ...
    def test_vital_signs_blood_pressure_exact_values(self, epic_bundle):
        ...
        if systolic:
            # Exact code
            loinc_coding = next(
                (c for c in systolic["code"]["coding"] if c["system"] == "http://loinc.org"), None
            )
            ...
            assert any(cat["coding"][0]["code"] == "vital-signs" for cat in systolic["category"])
    ...
    def test_organization_exact_values(self, epic_bundle):
        ...
        oid_ident = next(
            (i for i in org["identifier"] if "1.3.6.1.4.1.16517" in i.get("system", "")), None
        )
    ...
    def test_device_exact_values(self, epic_bundle):
        ...
        devices = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Device"
        ]
    ...
    def test_location_exact_values(self, epic_bundle):
        ...
        locations = [
            e.resource for e in epic_bundle.entry if e.resource.get_resource_type() == "Location"
        ]

        if len(locations) > 0:
            loc = (
                locations[0].dict() if hasattr(locations[0], "dict") else locations[0].model_dump()
            )
