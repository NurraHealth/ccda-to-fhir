...
class TestNISTDetailedValidation:
        ...
        for condition in conditions:
            if condition.code and condition.code.coding:
                for coding in condition.code.coding:
                    if (
                        coding.code == "233604007"
                        and coding.system == "http://snomed.info/sct"
                        and condition.clinicalStatus
                        and "resolved" in condition.clinicalStatus.coding[0].code
                    ):
                        pneumonia_resolved = condition
                        break

        assert pneumonia_resolved is not None, (
            "Must have Condition with SNOMED code 233604007 (pneumonia) with resolved status"
        )
    ...
    def test_problem_asthma_active(self, nist_bundle):
        ...
        assert asthma.category is not None and len(asthma.category) > 0, (
        ...
        assert any(
            code in ["problem-list-item", "encounter-diagnosis"] for code in category_codes
    ...
    def test_allergy_penicillin_with_hives(self, nist_bundle):
        ...
        assert penicillin is not None, (
            "Must have AllergyIntolerance with RxNorm code 7982 (penicillin)"
        )
        ...
        if penicillin.category:
            assert "medication" in penicillin.category, (
                "Penicillin allergy category should be 'medication'"
            )
    ...
    def test_allergy_codeine_with_shortness_of_breath(self, nist_bundle):
        ...
        # EXACT check: Code text
        assert "codeine" in codeine.code.text.lower(), "AllergyIntolerance must mention 'codeine'"
    ...
    def test_allergy_aspirin_with_hives(self, nist_bundle):
        ...
        # EXACT check: Code text
        assert "aspirin" in aspirin.code.text.lower(), "AllergyIntolerance must mention 'aspirin'"
            ...
            if reaction.manifestation and len(reaction.manifestation) > 0:
                ...
                if manifestation.text:
                    assert "hives" in manifestation.text.lower(), (
                        "Aspirin reaction must mention 'hives'"
                    )
    ...
    def test_medication_albuterol_inhalant(self, nist_bundle):
        ...
        if albuterol.medicationReference:
            ...
            for entry in nist_bundle.entry:
                if (
                    entry.resource.get_resource_type() == "Medication"
                    and entry.resource.id in albuterol.medicationReference.reference
                ):
            ...
            if med_resource and med_resource.code:
                ...
                rxnorm_code = next(
                    (c.code for c in med_resource.code.coding if c.code == "573621"), None
                )
                if rxnorm_code:
    ...
    def test_encounter_inpatient(self, nist_bundle):
        ...
        assert "2012-08-06" in str(inpatient.period.start), (
            "Encounter period.start must be 2012-08-06"
        )
    ...
    def test_diagnostic_report_cbc(self, nist_bundle):
        ...
        assert cbc is not None, (
            "Must have DiagnosticReport with SNOMED code 43789009 (CBC WO DIFFERENTIAL)"
        )
    ...
    def test_medication_has_route_and_dose_and_rate(self, nist_bundle):
        ...
        assert albuterol is not None, (
            "Must have MedicationStatement with RxNorm code 573621 (Albuterol)"
        )
        ...
        route_code = next((c.code for c in dosage.route.coding if c.code == "C38216"), None)
        assert route_code == "C38216", "Albuterol route must be 'C38216' (RESPIRATORY INHALATION)"

        # EXACT check: Display name
        route_display = next((c.display for c in dosage.route.coding if c.code == "C38216"), None)
    ...
    def test_immunization_has_route_dose_and_manufacturer(self, nist_bundle):
        ...
        route_code = next((c.code for c in flu_shot.route.coding if c.code == "C28161"), None)
        assert route_code == "C28161", "Influenza route must be 'C28161' (Intramuscular)"

        # EXACT check: Route display
        route_display = next((c.display for c in flu_shot.route.coding if c.code == "C28161"), None)
        assert route_display is not None, "Route must have display name"
    ...
    def test_encounter_has_period_with_dates(self, nist_bundle):
        ...
        for enc in encounters:
            if (
                ...
                and enc.period.end
                and "2012-08-06" in str(enc.period.start)
                and "2012-08-13" in str(enc.period.end)
            ):
    ...
    def test_procedure_has_performer_with_contact(self, nist_bundle):
        ...
        # Find all Procedures
        procedures = [
            e.resource for e in nist_bundle.entry if e.resource.get_resource_type() == "Procedure"
        ]
        ...
        if len(procedures) > 0:
            ...
            if procedure_with_performer:
                ...
                if performer.actor.reference:
                    ...
                    for entry in nist_bundle.entry:
                        if (
                            entry.resource.get_resource_type() == "Practitioner"
                            and entry.resource.id in performer.actor.reference
                        ):
                            ...
                            if practitioner.address and len(practitioner.address) > 0:
                                ...
                                assert addr.state == "OR", "Practitioner state must be 'OR'"
    ...
    def test_patient_has_marital_status(self, nist_bundle):
        ...
        marital_code = next(
            (
                c.code
                for c in patient.maritalStatus.coding
    ...
    def test_patient_has_communication_with_mode(self, nist_bundle):
        ...
        lang_code = next((c.code for c in comm.language.coding), None)
        assert lang_code == "eng", f"Communication language code must be 'eng', got '{lang_code}'"

        # EXACT check: preferred=true
        assert comm.preferred is True, f"Communication preferred must be true, got {comm.preferred}"
        ...
        if comm.extension and len(comm.extension) > 0:
            ...
            if proficiency_ext and proficiency_ext.extension:
                type_ext = next(
                    (ext for ext in proficiency_ext.extension if ext.url == "type"), None
                )
    ...
    def test_patient_has_contact_guardian(self, nist_bundle):
        ...
        # EXACT check: contact
        assert patient.contact is not None and len(patient.contact) > 0, "Patient must have contact"
        ...
        for allergy in allergies:
            if allergy.code and allergy.code.coding:
                for coding in allergy.code.coding:
                    ...
                    if coding.system and "rxnorm" in coding.system.lower() and allergy.category:
    ...
    def test_patient_references_organization(self, nist_bundle):
        ...
        if (
            patient
            and org
            and hasattr(patient, "managingOrganization")
            ...
            if hasattr(encounter, "diagnosis") and encounter.diagnosis:
                ...
                for diagnosis in encounter.diagnosis:
                    ...
                    condition_exists = any(
                        e.resource.get_resource_type() == "Condition"
                        and e.resource.id == condition_id
                        for e in nist_bundle.entry
                    )
    ...
    def test_procedures_reference_patient(self, nist_bundle):
        ...
        if len(procedures) > 0:
            for procedure in procedures:
                ...
                assert procedure.subject.reference is not None, (
                    "Procedure.subject must have reference"
                )
    ...
    def test_medication_requests_reference_patient(self, nist_bundle):
        ...
        for med_statement in med_statements:
            ...
            assert med_statement.subject.reference is not None, (
                "MedicationStatement.subject must have reference"
            )
    ...
    def test_medication_statements_have_status(self, nist_bundle):
        ...
        for ms in med_statements:
            assert ms.status is not None, "MedicationStatement.status is required"
            assert ms.status in [
                "active",
                "completed",
                "entered-in-error",
    ...
    def test_medication_has_reason_code(self, nist_bundle):
        ...
        pneumonia_code = next((c for c in reason.coding if c.code == "233604007"), None)
    ...
    def test_medication_has_requester_practitioner(self, nist_bundle):
        ...
        # Check given name
        assert name.given is not None and len(name.given) > 0, "Practitioner must have given name"
        assert "Henry" in name.given, f"Practitioner given name must be 'Henry', got '{name.given}'"
    ...
    def test_encounter_has_location_with_details(self, nist_bundle):
        ...
        assert encounter_with_location is not None, "At least one Encounter must have location"
        ...
        assert location.type is not None and len(location.type) > 0, "Location must have type"
        ...
        urgent_care_code = next((c for c in location_type.coding if c.code == "1160-1"), None)
    ...
    def test_procedures_have_performed_datetime(self, nist_bundle):
        ...
        if len(procedures) > 0:
            for procedure in procedures:
                has_performed = (
                    (
                        hasattr(procedure, "performedDateTime")
                        and procedure.performedDateTime is not None
                    )
                    or (
                        hasattr(procedure, "performedPeriod")
                        and procedure.performedPeriod is not None
                    )
                    or (
                        hasattr(procedure, "performedString")
                        and procedure.performedString is not None
                    )
                )
    ...
    def test_procedures_have_code_display_values(self, nist_bundle):
        ...
        if len(procedures) > 0:
            for procedure in procedures:
                assert procedure.code is not None, "Procedure must have code"
