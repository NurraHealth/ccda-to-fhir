...
class TestCernerDetailedValidation:
    ...
    def test_patient_steve_williamson_demographics(self, cerner_bundle):
        ...
        assert patient.identifier is not None and len(patient.identifier) > 0, (
            "Patient must have identifier"
        )
        mrn = patient.identifier[0]
        assert mrn.value == "106", "Patient MRN must be '106'"
        assert "2.16.840.1.113883.1.13.99999.1" in mrn.system, (
            "Patient identifier must have Cerner system OID"
        )

        # EXACT check: Address
        assert patient.address is not None and len(patient.address) > 0, "Patient must have address"
        addr = patient.address[0]
        assert "8745 W Willenow Rd" in addr.line, (
            "Patient address line must be '8745 W Willenow Rd'"
        )
    ...
    def test_problem_angina(self, cerner_bundle):
        ...
        assert "angina" in angina.code.text.lower(), "Condition must mention 'angina'"
        ...
        assert "2013-07-10" in str(angina.recordedDate), (
            "Condition recorded date must be 2013-07-10"
        )

        # EXACT check: category
        assert angina.category is not None and len(angina.category) > 0, (
            "Condition must have category"
        )
        cat_coding = angina.category[0].coding[0]
        assert cat_coding.code == "problem-list-item", (
            "Condition category must be 'problem-list-item'"
        )
    ...
    def test_problem_diabetes_type_2(self, cerner_bundle):
        ...
        assert diabetes is not None, (
            "Must have Condition with SNOMED code 44054006 (diabetes type 2)"
        )
    ...
    def test_problem_hypercholesterolemia(self, cerner_bundle):
        ...
        assert hyperchol is not None, (
            "Must have Condition with SNOMED code 13644009 (hypercholesterolemia)"
        )

        # EXACT check: Code text
        assert "cholesterol" in hyperchol.code.text.lower(), "Condition must mention 'cholesterol'"
    ...
    def test_allergy_codeine_with_reaction(self, cerner_bundle):
        ...
        assert "2013-07-10" in str(codeine.recordedDate), (
            "AllergyIntolerance recorded date must be 2013-07-10"
        )

        # EXACT check: Reaction manifestation (Nausea) and severity
        assert codeine.reaction is not None and len(codeine.reaction) > 0, (
            "AllergyIntolerance must have reaction"
        )
    ...
    def test_allergy_penicillin_with_reaction(self, cerner_bundle):
        ...
        assert penicillin is not None, (
            "Must have AllergyIntolerance with RxNorm code 7980 (penicillin)"
        )
        ...
        if penicillin.reaction and len(penicillin.reaction) > 0:
            ...
            if reaction.manifestation and len(reaction.manifestation) > 0:
                ...
                if manifestation.text:
                    ...
                    assert (
                        "weal" in manifestation.text.lower() or "hive" in manifestation.text.lower()
    ...
    def test_composition_has_all_expected_sections(self, cerner_bundle):
        ...
        for section in composition.section:
            if section.code and section.code.coding:
                ...
                if (
                    code in expected_sections
                    and code not in ["11369-6"]  # Skip immunizations check
                    and section.entry is not None
                ):
    ...
    def test_encounter_ambulatory(self, cerner_bundle):
        ...
        assert "2013-07-10" in str(ambulatory.period.start), (
    ...
    def test_procedure_electrocardiographic(self, cerner_bundle):
        ...
        assert ekg is not None, (
            "Must have Procedure with SNOMED code 29303009 (Electrocardiographic procedure)"
        )
        ...
        assert "2013-07-10" in str(ekg.performedDateTime), (
            "Procedure performed date must be 2013-07-10"
        )
        ...
        for mr in med_requests:
            if mr.medicationCodeableConcept and mr.medicationCodeableConcept.coding:
                for coding in mr.medicationCodeableConcept.coding:
                    if (
                        coding.code == "311041"
                        and coding.system == "http://www.nlm.nih.gov/research/umls/rxnorm"
                    ):
                        insulin = mr
                        break

        assert insulin is not None, (
            "Must have MedicationRequest with RxNorm code 311041 (Insulin Glargine)"
        )
    ...
    def test_observation_lab_result_with_value_and_units(self, cerner_bundle):
        ...
        assert obs_with_value is not None, (
            "Must have at least one laboratory Observation with valueQuantity"
        )
        ...
        assert (
            obs_with_value.interpretation is not None and len(obs_with_value.interpretation) > 0
    ...
    def test_medication_request_has_authored_on(self, cerner_bundle):
        ...
        assert "2013-07-10" in str(insulin.authoredOn), "AuthoredOn date must be 2013-07-10"
        assert "21:58:10" in str(insulin.authoredOn), "AuthoredOn time must be 21:58:10"
    ...
    def test_encounter_has_location(self, cerner_bundle):
        ...
        assert "97005" in location.address.postalCode, "Location postal code must include '97005'"
    ...
    def test_observation_blood_pressure_has_components(self, cerner_bundle):
        ...
        assert bp_panel.effectiveDateTime is not None, (
            "Blood pressure observation must have effectiveDateTime"
        )
    ...
    def test_medication_requests_have_dispense_request_when_supply_present(self, cerner_bundle):
        ...
        assert insulin.dispenseRequest.quantity.value == 10, "Supply quantity should be 10"
        assert insulin.dispenseRequest.quantity.unit == "mL", "Supply unit should be mL"
    ...
    def test_all_medication_requests_have_status(self, cerner_bundle):
        ...
        for mr in med_requests:
            assert mr.status is not None, "MedicationRequest.status is required (FHIR)"
            assert mr.status in [
                "active",
                "on-hold",
                "cancelled",
    ...
    def test_medication_request_has_dosage_instruction_text(self, cerner_bundle):
        ...
        requests_with_dosage = [
            mr
            for mr in med_requests
            if hasattr(mr, "dosageInstruction")
            and mr.dosageInstruction is not None
            and len(mr.dosageInstruction) > 0
        ]
