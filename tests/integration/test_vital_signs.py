...
class TestVitalSignsConversion:
    ...
    def test_converts_panel_code(self, ccda_vital_signs: str, fhir_vital_signs: JSONObject) -> None:
    ...
    def test_converts_category(self, ccda_vital_signs: str, fhir_vital_signs: JSONObject) -> None:
        ...
        assert (
            panel["category"][0]["coding"][0]["system"]
    ...
    def test_converts_heart_rate(self, ccda_vital_signs: str, fhir_vital_signs: JSONObject) -> None:
...
<ClinicalDocument xmlns="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <component>
        ...
        hr_obs = next(
            (
                ...
                if obs.get("code", {}).get("coding", [{}])[0].get("code") == "8867-4"
            ),
            None,
        )
    ...
    def test_converts_method_code_oral_temperature(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_temp_oral_method.xml", encoding="utf-8"
        ) as f:
    ...
    def test_converts_method_code_axillary_temperature(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_temp_axillary_method.xml", encoding="utf-8"
        ) as f:
            ccda_vital_signs = f.read()
        ...
        assert coding["display"] == "Axillary temperature taking", (
            "Method display should be preserved"
        )
    ...
    def test_method_code_not_present_when_absent(self) -> None:
        ...
        assert "method" not in hr_obs, (
            "Observation should not have method field when methodCode is absent"
        )
    ...
    def test_converts_body_site_blood_pressure(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_bp_with_body_site.xml", encoding="utf-8"
        ) as f:
            ccda_vital_signs = f.read()
    ...
    def test_converts_body_site_heart_rate(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_hr_with_body_site.xml", encoding="utf-8"
        ) as f:
            ccda_vital_signs = f.read()
    ...
    def test_body_site_not_present_when_absent(self) -> None:
        ...
        assert "bodySite" not in hr_obs, (
            "Observation should not have bodySite field when targetSiteCode is absent"
        )
    ...
    def test_converts_body_site_with_laterality_qualifier_blood_pressure(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_bp_with_laterality.xml", encoding="utf-8"
        ) as f:
            ccda_vital_signs = f.read()
        ...
        assert site_coding["code"] == "40983000", (
            "Body site code should be 40983000 (Upper arm structure)"
        )
        assert site_coding["display"] == "Upper arm structure", (
            "Body site display should be preserved"
        )
    ...
    def test_converts_body_site_with_laterality_qualifier_heart_rate(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_hr_with_laterality.xml", encoding="utf-8"
        ) as f:
            ccda_vital_signs = f.read()
        ...
        assert site_coding["code"] == "45631007", (
            "Body site code should be 45631007 (Structure of radial artery)"
        )
        assert site_coding["display"] == "Structure of radial artery", (
            "Body site display should be preserved"
        )
    ...
    def test_converts_interpretation_code_normal(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_with_interpretation.xml", encoding="utf-8"
        ) as f:
            ccda_vital_signs = f.read()
    ...
    def test_converts_interpretation_code_critical_high(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_critical_interpretation.xml",
            encoding="utf-8",
        ) as f:
            ccda_vital_signs = f.read()
    ...
    def test_interpretation_code_not_present_when_absent(self) -> None:
        ...
        assert "interpretation" not in hr_obs, (
            "Observation should not have interpretation field when interpretationCode is absent"
        )
    ...
    def test_converts_reference_range_individual_vital_sign(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_hr_with_reference_range.xml",
            encoding="utf-8",
        ) as f:
            ccda_vital_signs = f.read()
        ...
        assert ref_range["low"]["system"] == "http://unitsofmeasure.org", (
            "Reference range should use UCUM system"
        )
    ...
    def test_converts_reference_range_blood_pressure_panel(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_bp_with_reference_ranges.xml",
            encoding="utf-8",
        ) as f:
            ccda_vital_signs = f.read()
        ...
        # Verify referenceRange field exists
        assert "referenceRange" in bp_obs, (
            "Blood pressure observation should have referenceRange field"
        )
        ref_ranges = bp_obs["referenceRange"]

        # Verify referenceRange is an array with two elements (systolic and diastolic)
        assert isinstance(ref_ranges, list), "referenceRange should be an array"
        assert len(ref_ranges) == 2, (
            "referenceRange should have two elements (systolic and diastolic)"
        )
    ...
    def test_reference_range_not_present_when_absent(self) -> None:
        ...
        assert "referenceRange" not in hr_obs, (
            "Observation should not have referenceRange field when absent in C-CDA"
        )
        ...
        assert "referenceRange" not in bp_obs, (
            "Blood pressure observation should not have referenceRange field when absent in C-CDA"
        )
    ...
    def test_reference_range_filters_for_normal_interpretation_code_only(self) -> None:
        ...
        with open(
            "tests/integration/fixtures/ccda/vital_signs_hr_with_multiple_reference_ranges.xml",
            encoding="utf-8",
        ) as f:
            ccda_vital_signs = f.read()
        ...
        assert len(ref_ranges) == 1, (
            "Should only include one reference range (Normal interpretation code)"
        )
